...
class GeneralJournalReport extends ModuleController
{
    ...
    public function search(Request $request)
    {
        ...
        $months = CarbonPeriod::create($from, '1 day', $to);
        if(sizeof($months) > 5){
        ...
        foreach ($months as $key => $dt) {
            ...
            $data_all[$key]['sale'] = SaleOrders::where('is_archive', 0)->where('status', 1)->whereYear('order_date', Carbon::parse($dt)->year)->whereMonth('order_date', Carbon::parse($dt)->month)->sum('grand_total');
            $data_all[$key]['discount'] = SaleOrders::where('is_archive', 0)->where('status', 1)->whereYear('order_date', Carbon::parse($dt)->year)->whereMonth('order_date', Carbon::parse($dt)->month)->sum('discount_total');
            $data_all[$key]['cost'] = PurchaseOrders::where('is_archive', 0)->where('status', 1)->whereYear('order_date', Carbon::parse($dt)->year)->whereMonth('order_date', Carbon::parse($dt)->month)->sum('grand_cost_total');
            $data_all[$key]['expense'] = Expenses::where('is_archive', 0)->whereYear('expense_date', Carbon::parse($dt)->year)->whereMonth('expense_date', Carbon::parse($dt)->month)->sum('amount');
