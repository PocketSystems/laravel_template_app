        ...
        <form action="{{route('module.'.$moduleName.'.search')}}" method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-5">
                    ...
                    <select class="form-control select2  @error('category_id') is-invalid @enderror" name="supplier_id">
                        <option label="Select Customer"></option>
                        @foreach ($suppliers as $value)
                            <option
                                value="{{$value['id']}}" {{\App\Helpers\Helper::reqValue('supplier_id') == $value['id'] ? 'selected' : ''}}>{{$value['name']}}</option>
                        @endforeach
                    </select>
                    @error('supplier_id')
                    <div class="tx-danger">{{ $message }}</div>
    ...
    @if(!empty(\request()->toArray()))
        ...
        <script>
            ...
            var soldTotal = $.plot('#soldTotal', [{
                data:
                {!!$purchaseTotalGraph!!}.map((data, index) => [index, getRndInteger(data.amount / dataFraction, data.amount / chartEnd)]),
                color: '#e5e9f2'
            }, {
                data: {!!$purchaseTotalGraph!!}.map((data, index) => [index, data.amount / dataFraction]),
                color: '#66a4fb'
            }], flotChartOption1);

            var reciveTotal = $.plot('#reciveTotal', [{
                data:
                {!!$paidTotalGraph!!}.map((data, index) => [index, getRndInteger(data.amount / dataFraction, data.amount / chartEnd)]),
                color: '#e5e9f2'
            }, {
                data: {!!$paidTotalGraph!!}.map((data, index) => [index, data.amount / dataFraction]),
                color: '#f10075'
            }], flotChartOption1);
